    ...
    <header role="banner">
      ...
      <div class="page-head">
        ...
        <div class="container">
          ...
          <h1 class="page-title">Common Errors</h1>
...
<a
  ...
  class="icon-edit"
  href="https://github.com/pouchdb/pouchdb/edit/master/docs/errors.md"
  target="_blank"
></a>
...
<p><a class='h3 anchor' href='#no_access_control_allow_origin_header' name='no_access_control_allow_origin_header'>
...
<p><a class='h3 anchor' href='#no_valid_adapter' name='no_valid_adapter'>
PouchDB throws a <code>No valid adapter found</code> error
</a></p>
...
<p><a class='h3 anchor' href='#not_enough_space' name='not_enough_space'>
...
<p><a class='h3 anchor' href='#404__local_document' name='404__local_document'>
...
<p><a class='h3 anchor' href='#throwing_invalidstateerror' name='throwing_invalidstateerror'>
PouchDB is throwing <code>InvalidStateError</code>
</a></p>
...
<p><a class='h3 anchor' href='#phonegap_cordova_second_database' name='phonegap_cordova_second_database'>
...
<div id='event_emitter_limit'></div>

<p><a class='h3 anchor' href='#event_emitter_limit' name='event_emitter_limit'>
Possible EventEmitter memory leak detected
</a></p>

<p>If you see this warning:</p>
<div class="highlight"><pre><code class="language-text" data-lang="text">(node) warning: possible EventEmitter memory leak detected. 11 listeners added.
Use emitter.setMaxListeners() to increase limit.
</code></pre></div>
<p>This is because PouchDB uses Node-style <a href="https://nodejs.org/api/events.html">EventEmitters</a> for its events. An EventEmitter is any object that has an <code>.on()</code> or <code>once()</code> method, such as <code>db.changes().on(&#39;change&#39;, ...</code>.</p>

<p>By default, all EventEmitters have 10 listeners, and if you exceed that limit, e.g. by attaching many <code>changes()</code> listeners or running many simultaneous <code>replicate()</code> or <code>sync()</code> events, then you may exceed this limit.</p>

<p><strong>This could indicate a memory leak in your code</strong>. Check to make sure that you are calling <code>cancel()</code> on any <code>changes()</code>, <code>replicate()</code>, or <code>sync()</code> handlers, if you are constantly starting and stopping those events.</p>

<p>If you&#39;re sure it&#39;s not a memory leak, though, you can increase the limit by doing:</p>

<div class="highlight"><pre><code class="language-javascript" data-lang="javascript"><span class="nx">db</span><span class="p">.</span><span class="nx">setMaxListeners</span><span class="p">(</span><span class="mi">20</span><span class="p">);</span>  <span class="c1">// or 30 or 40 or however many you need</span></code></pre></div>

<p>In the above example, <code>db</code> refers to a database object you created using <code>new PouchDB(&#39;dbname&#39;)</code>.</p>

<div id='size_limitation_5mb'></div>

<p><a class='h3 anchor' href='#size_limitation_5mb' name='size_limitation_5mb'>
...
<p><a class='h3 anchor' href='#404_get_couchapp' name='404_get_couchapp'>
...
<p><a class='h3 anchor' href='#typeerror_object_is_not_a_function' name='typeerror_object_is_not_a_function'>
...
<p><a class='h3 anchor' href='#undefined_is_not_a_function' name='undefined_is_not_a_function'>
...
<p><a class='h3 anchor' href='#cordova_android_parse_error' name='cordova_android_parse_error'>
...
<p><a class='h3 anchor' href='#safari_object_silent_fail' name='safari_object_silent_fail'>
...
<p><a class='h3 anchor' href='#window_localstorage_chrome_apps' name='window_localstorage_chrome_apps'>
...
<p><a class='h3 anchor' href='#unknown_error_ff' name='unknown_error_ff'>
...
<p><a class='h3 anchor' href='#unknown_error_chrome' name='unknown_error_chrome'>
...
<p><a class='h3 anchor' href='#could_not_be_cloned' name='could_not_be_cloned'>
...
<p><a class='h3 anchor' href='#android_pre_kitkat' name='android_pre_kitkat'>
DOM Exception 18 in Android pre-Kitkat WebView
</a></p>
...
<p><a class='h3 anchor' href='#windows_leveldown' name='windows_leveldown'>
PouchDB on Windows
</a></p>
...
<p>Also, you have the option to use <a href="%3E=%201.2.2">leveldown</a>(https://github.com/level/leveldown), which avoids native leveldown building when installing. Make sure the <code>win32-x64.tar.gz</code> is uploaded for your <a href="https://github.com/Level/leveldown/releases">leveldown</a> version. Then for use leveldown as backend, you can install:</p>

<div class="highlight"><pre><code class="language-bash" data-lang="bash">npm install leveldown@1.2.2</code></pre></div>

<p>and instance your PouchDB like this:</p>
<div class="highlight"><pre><code class="language-js" data-lang="js"><span class="kd">var</span> <span class="nx">db</span> <span class="o">=</span> <span class="k">new</span> <span class="nx">PouchDB</span><span class="p">(</span><span class="s1">&#39;database&#39;</span><span class="p">,</span> <span class="p">{</span> <span class="nx">db</span><span class="o">:</span> <span class="nx">require</span><span class="p">(</span><span class="s1">&#39;leveldown&#39;</span><span class="p">)</span> <span class="p">});</span>
</code></pre></div>
<div id='replicating_attachments_slow'></div>

<p><a class='h3 anchor' href='#replicating_attachments_slow' name='replicating_attachments_slow'>
Replication with attachments is slow or fails
</a></p>
...
<div id='package_pouchdb_webpack'></div>

<p><a class='h3 anchor' href='#package_pouchdb_webpack' name='package_pouchdb_webpack'>
Packaging PouchDB in an app with WebPack
</a></p>

<p>PouchDB may have various dependencies that may not play nicely with WebPack. Here are some issues you may run into and their resolutions:</p>

<p><strong>You may need an appropriate loader to handle this file type.</strong></p>

<p>If you run into the following error (or similar):</p>
<div class="highlight"><pre><code class="language-sh" data-lang="sh">ERROR in ./~/pouchdb/~/levelup/package.json
...
<span class="p">|</span>   <span class="s2">&quot;name&quot;</span>: <span class="s2">&quot;levelup&quot;</span>,
<span class="p">|</span>   <span class="s2">&quot;description&quot;</span>: <span class="s2">&quot;Fast &amp; simple storage - a Node.js-style LevelDB wrapper&quot;</span>,
<span class="p">|</span>   <span class="s2">&quot;version&quot;</span>: <span class="s2">&quot;0.18.6&quot;</span>,
 @ ./~/pouchdb/~/levelup/lib/util.js 102:30-56
</code></pre></div>
<p>WebPack needs to be configured to recognize how to load json files. Simply, install <code>json-loader</code> and edit <code>webpack.config.js</code> as follows: </p>
<div class="highlight"><pre><code class="language-js" data-lang="js"><span class="nx">module</span><span class="o">:</span> <span class="p">{</span>
    <span class="nx">loaders</span><span class="o">:</span> <span class="p">[</span>
        <span class="c1">// https://github.com/pouchdb/pouchdb/issues/3319</span>
        <span class="p">{</span>
            <span class="nx">test</span><span class="o">:</span> <span class="sr">/\.json$/</span><span class="p">,</span>
            <span class="nx">loader</span><span class="o">:</span> <span class="s2">&quot;json-loader&quot;</span>
        <span class="p">}</span>
    <span class="p">]</span>
<span class="p">}</span>
</code></pre></div>
<div id='couchbase_dbname'></div>

<p><a class='h3 anchor' href='#couchbase_dbname' name='couchbase_dbname'>
Failed to load resource: the server responded with a status of 400 (Bad request) 
</a></p>

<p>If you are using Couchbase Lite to sync with PouchDB then you cannot use capital letters in your database name as Couchbase Lite has restrictions on valid database names.</p>
